> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [www.drawio.com](https://www.drawio.com/doc/faq/embed-mode)

> If the embed=1 URL parameter is used, the client runs in embed mode and the page will send a “ready” ......
...
*   `modified=0`: Disables the update of the modified state in embed mode after _Save_ is selected and enables the update of the status message after changes are made. If `0` is used, the status bar is cleared after the changes. Otherwise, the value of this is used as the resource key. Instead of using this [URL parameter](https://www.drawio.com/doc/faq/supported-url-parameters.html), you can specify this setting in the load message.
...
If the `configure=1` URL parameter is used, the client sends `{event: 'configure'}` and waits for `{action: 'configure', config: obj}` before creating the main application and sending the `init` event. The `obj` is [specified in the configuration](https://www.drawio.com/doc/faq/configure-diagram-editor), e.g. `config: {defaultFonts: ["Humor Sans"]}`.
...
**Note:** For XML, all supported file formats can be used, including PNG+XML, the data part of an SVG data URI with UTF8 encoding, or the complete SVG or PNG data URI with base64 encoding. For all `.vsd*` files, a data URI with a `data:application/vnd.visio;base64` prefix must be used. Lucidchart and Gliffy files are represented as a JSON string.
...
*   [Simple embedding walkthrough](https://www.drawio.com/blog/embedding-walkthrough.html)