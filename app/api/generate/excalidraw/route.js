...
/**
 * POST /api/generate/excalidraw
 * Generate Excalidraw JSON based on user input
 */
export async function POST(request) {
  try {
    ...
    // Build current user message (with optional multimodal)
    let userMessage;

    if (typeof userInput === 'object' && (userInput.image || userInput.images || userInput.text)) {
      const { text, image, images } = userInput;
      const imageArray = images || (image ? [image] : []);
      const baseText = typeof text === 'string' ? text : (typeof userInput === 'string' ? userInput : '');

      userMessage = {
        role: 'user',
        content: USER_PROMPT_TEMPLATE(baseText || '', chartType),
